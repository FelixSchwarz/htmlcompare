...
- both spellings of a URL are the same: `background: url(a.png)`, `background: url("a.png")` and `background: url('a.png')` declare the same background, and the function name is case-insensitive like every other one (`URL(a.png)`). This holds wherever a URL may appear, `@import url(a.css)` included — except inside a custom property, whose value stays verbatim as described above.
